...
rl.on("line", async input => {
  ...
  for (let i = 0; i < stages.length; i++) {
    ...
    const { stdout: redirectedStdout, stderr: redirectedStderr } = getStreamTargets(command.args);

    const isLastStage = i === stages.length - 1;
    const nextPipe = !isLastStage && !redirectedStdout ? new PassThrough() : undefined;
